...
function generateColorSuggestions(
  ...
  const targetAAA = textSize === "large" ? 4.5 : 7; // reserved for future AAA checks
  ...
  const unique = suggestions.filter(
    (s, i, arr) => arr.findIndex((t) => t.color === s.color) === i,
  );

  return unique
    .sort(
      (a, b) =>
        colorDistance(foreground, a.color) - colorDistance(foreground, b.color),
    )
    .slice(0, 3);
}
 ...
 */
function extractDarkModeColors(
  html: string,
): Array<{ fg: string; bg: string; element: string }> {
 ...
 */
function analyzeModeContrast(html: string, mode: ColorMode): ContrastIssue[] {
  ...
  colorPairs.forEach(({ fg, bg, element }) => {
    ...
    if (!wcagAA) {
      ...
      const recommendation = generateContrastRecommendation(
        ratio,
        textSize,
        fg,
        bg,
 ...
 */
function extractLightModeColors(
  html: string,
): Array<{ fg: string; bg: string; element: string }> {
  ...
  const textElementRegex =
    /<(p|h[1-6]|a|button|span|div|li)[^>]*style="([^"]*)"[^>]*>([^<]*)<\/\1>/gi;
  ...
  while ((match = textElementRegex.exec(html)) !== null) {
    ...
    // Create regex inside loop to avoid shared lastIndex bug
...
function generateModeReport(
  ...
  return {
    ...
    wcagAA: {
      ...
      percentage:
        issues.length > 0
          ? Math.round((wcagAAPass / issues.length) * 100)
          : 100,
    },
    wcagAAA: {
      pass: wcagAAAPass,
      ...
      percentage:
        issues.length > 0
          ? Math.round((wcagAAAPass / issues.length) * 100)
          : 100,
    },
  };
}
 ...
 */
export async function analyzePageContrast(
  html: string,
  url: string,
  ...
  try {
    ...
    const report: AccessibilityReport = {
      ...
      wcagAA: {
        ...
        percentage:
          allIssues.length > 0
            ? Math.round((wcagAAPass / allIssues.length) * 100)
            : 100,
      },
      wcagAAA: {
        pass: wcagAAAPass,
        ...
        percentage:
          allIssues.length > 0
            ? Math.round((wcagAAAPass / allIssues.length) * 100)
            : 100,
      },
    };
 ...
 */
function generateDualModeSummary(
  lightReport: ModeReport,
  darkReport: ModeReport,
): string {
  ...
  if (lightAA >= 90 && darkAA >= 90) {
    ...
    summary =
      "Good accessibility in both modes, with minor improvements needed.";
  } else if (lightAA >= 80 || darkAA >= 80) {
...
export function suggestImprovedColors(
  ...
  if (suggestions.length > 0) {
    const best =
      suggestions.find((s) => s.ratio >= targetRatio) || suggestions[0];
    return {
